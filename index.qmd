...
presidential_clean <- presidential |>
  ...
  select(casilla_code, acta_code, entity_state, casilla_type, casilla, independent, null_votes, total_votes, eligible_voters, sheinbaum, galvez, maynez)
...
crimes_messy|> # we only want crime data from 2023 as it was before the election, possible avg after 2018
  mutate(
    State = str_replace(State, "Ciudad de México", "Distrito Federal"),
    State = str_replace(State, " de Zaragoza", ""),
    State = str_replace(State, "Estado de ", ""),
...
# indigenous mexicans (by state, those who speak a language)
...
# import second indigenous dataset idfk
library(readxl)
...
indigenous_pop_24 <- indigenous_messy |>
  ...
  rename(
    code_state = `Clave de la entidad`,
    state = Entidad,
    pop_ind = Población,
    pct_ind = Porcentaje
  )
