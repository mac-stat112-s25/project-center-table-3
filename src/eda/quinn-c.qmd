...
presidential |>
  ...
  ggplot(aes(x = vote_abroad, y = votes, fill = candidate)) +
    ...
    theme_light() +
  labs(x = "Voted Abroad?", y = "Vote Share", fill = "Candidate", caption = "By Quinn Cowell, 4/27/2025, INE 2024", title = "Abroad voting vs. voting within mexico")
```
...
presidential |>
  ...
  ggplot(aes(x = turnout)) +
    geom_density() +
    theme_light() +
    labs(x = "Voter Turnout by Polling Place")
```
...
mexico |>
  ...
  ggplot() +
    ...
    theme_void() +
    scale_fill_manual(values = brewer.pal(9, "Reds"), name = "% Vote")
```
```{r}
# plot Gálvez results
mexico |>
  ...
  ggplot() +
    ...
    theme_void() +
    scale_fill_manual(values = brewer.pal(8, "RdPu"), name = "% Vote")
```
```{r}
# plot Máynez results
mexico |>
  ...
  ggplot() +
    ...
    theme_void() +
    scale_fill_manual(values = brewer.pal(6, "Oranges"), name = "% Vote")
```
